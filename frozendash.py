...
df_daily.columns = df_daily.columns.astype(str)

# Correct July daily column detection
july_cols = [col for col in df_daily.columns if 'Jul' in col and any(char.isdigit() for char in col)]
...
df['FR'] = pd.to_numeric(df['FR'], errors='coerce').fillna(0)
...
st.write(df[['product_id', 'Jul', 'Total July Sales', 'DOI']].head(10))
